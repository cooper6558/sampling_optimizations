...
def main():
    ...
    if(len(sys.argv) == 9):
        ...
        True_Value_Folder_Path = "NA" #  no SNR
    ...
    # create vtp files from binary samples
    if (os.path.isdir(in_file_path)): # given a folder
        ...
        num_files = int(len(filenames_list))
    ...
    cur_samp = "linear"
    input_folder = in_file_path+"/vtp_files"
    ...
    if True_Value_Folder_Path != 'NA':
        SNR_Analysis_Path = "/home/mlhickm/data_sampling/reuse_sampler/compute_SNR.py"

        '''
        in_files = []
        for (dirpath, dirnames, filenames) in walk(True_Value_Folder_Path):
            for filename in filenames:
                #if (filename.startswith( input_file_prefix )):
                if (filename.endswith( ".vti" )):
                    in_files.append(filename)
            break
        # Sort list by number in file name
        ...
        contents = os.listdir(os.path.join(".",in_file_path+"/vtp_files"))
        ...
        statistics_out_file = in_file_path+"/vtp_files/SNR_results.csv"
        ...
        for timestep in range(num_files):
            ...
            if len(ROI_table) >= num_files: # if we correctly have the same number of ROIs as we do time-steps
                ...
                cmd = "python {} {} {}".format(SNR_Analysis_Path, True_Value_Folder_Path+"/"+filenames_list_true[timestep], in_file_path+"/vtp_files/"+filenames_list_recons[timestep])
                out_str = subprocess.check_output(cmd, shell=True)
                SNR = float((((str(out_str)).split()[-1]))[:-3])
                cmd = "python {} {} {} {} {} {} {} {} {}".format(SNR_Analysis_Path, True_Value_Folder_Path+"/"+filenames_list_true[timestep], in_file_path+"/vtp_files/"+filenames_list_recons[timestep], x_start, x_end, y_start, y_end, z_start, z_end)
                out_str = subprocess.check_output(cmd, shell=True)
